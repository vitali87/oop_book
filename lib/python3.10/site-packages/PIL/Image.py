...
MAX_IMAGE_PIXELS = int(1024**2 * 1024 // 4 // 3)
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def getexif(self):
        ...
        # XMP tags
        if (
            0x0112 not in self._exif
            and (xmp_tags := self.info.get("XML:com.adobe.xmp"))
            and (match := re.search(r'tiff:Orientation="([0-9])"', xmp_tags))
        ):
...
def _apply_env_variables(env=None):
    ...
    for var_name, setter in [
        ...
        units = 1
        for postfix, mul in [("k", 1024), ("m", 1024**2)]:
...
class Exif(MutableMapping):
    ...
    def _get_merged_dict(self):
        ...
        # get EXIF extension
        if 0x8769 in self and (ifd := self._get_ifd_dict(self[0x8769])):
